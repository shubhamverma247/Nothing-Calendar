...
- 2026-07-14: Open Testing privacy policy update completed. In-app Privacy Policy now accurately discloses that calendar content stays local while Google Play services are used for Billing, purchase restore, and in-app update checks; it also documents Internet/Network State use, the local Pro entitlement flag, and Nothing Glyph SDK integration without claiming DotCal has no third-party dependencies or no network permission. Effective date updated to July 14, 2026. DotCal-Site privacy/landing copy was updated in `D:\Caveman\caveman\DotCal-Site` to match the app policy and current Pro/Billing model. No Room schema/package/deep-link/DB/calendar behavior changes. `:app:testDebugUnitTest` and `:app:assembleDebug` passed. No ADB device connected, so debug APK was not installed. No manual phone UI QA run.
- 2026-07-14: Open Testing Glyph `NothingKey` removal confirmed. Removed only the app-level `NothingKey` manifest metadata; Glyph Toy service, `com.nothing.glyph.TOY` intent filter, foreground-service declaration, Glyph SDK permission, runtime guards, package id, deep links, Room schema, billing, sync, and app behavior unchanged. `:app:testDebugUnitTest` and `:app:assembleDebug` passed, debug APK installed successfully on Nothing Phone 3 device `000153573000720`, and user confirmed DotCal Glyph Toy still works without `NothingKey`. No manual phone UI QA beyond the user Glyph check.
- 2026-07-14: Open Testing release-prep sync failure copy fix completed. CalendarProvider sync failure toast no longer says `Check your internet connection`; permission-denied sync failures now say `Check calendar access`, while other sync failures say `Try again`, matching DotCal's local/offline sync model. No Room schema/package/deep-link/DB/billing/onboarding behavior changes. `:app:testDebugUnitTest` and `:app:assembleDebug` passed, debug APK installed successfully on device `4ab0d020`. No manual phone UI QA run.
...
Latest status: `versionCode 10` / `versionName 1.1.3`; `:app:testDebugUnitTest` and `:app:assembleDebug` passing. Latest debug APK installed successfully on Nothing Phone 3 device `000153573000720`. Latest completed release-prep changes are the sync failure copy fix and confirmed `NothingKey` manifest metadata removal. Agenda has Pro-only long-press multi-select with bulk move/copy/shift/calendar/color/ghost/delete actions and snackbar UNDO; the snackbar host now sits above the floating bottom nav, uses the active DotCal theme/accent, and auto-dismisses after about 4 seconds unless `UNDO` is tapped. No manual phone UI QA run unless explicitly asked.
